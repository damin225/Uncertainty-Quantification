...
count = 0;
for i=6000:2000:Nosample-2000
    count = count+1;
    E1 = nonzeros(store(i:i+2000,1));
    E2 = nonzeros(store(i:i+2000,2));
    E3 = nonzeros(store(i:i+2000,3));
    E4 = nonzeros(store(i:i+2000,4));
    sigma = nonzeros(store(i:i+2000,5));
    KL(1,count) = KLval(E1,28000,3000);
    KL(2,count) = KLval(E2,30000,3000);
    KL(3,count) = KLval(E3,32000,3000);
    KL(4,count) = KLval(E4,34000,3000);
    KL(5, count) = KLval(sigma,0.1,0.01);
end

cul_sample = [8000:2000:Nosample];

for i=6:10
    figure(i)
    plot(cul_sample,KL(i-5,:),'linewidth',2,'Marker','o');
    xlabel('cumlative samples numbers');
